...
1. **Библиотека (open source)** — чистое вычисление: `get_plotlines(episodes) → PlotterResult`. Синопсисы на вход, структурированные линии на выход. Без БД, без интерфейса, без файлов. `pip install plotter` — и работает. Это то, что выкладываем.

2. **Приложение (не open source)** — использует библиотеку внутри. Хранит результаты в базе, показывает в интерфейсе, даёт редактировать, объединять, удалять. Это то, что ты используешь сейчас в mas4bw. Остаётся твоим продуктом. Переписывается с нуля на основе идей из статьи Balestri & Pescatore (ICAART 2025), но с собственной теоретической базой и классификацией.
