...
Имя = абстрактное слово по ЦЕЛИ, не по событию. Episodic-линия (franchise engine): назови по формуле franchise — «Case of the Week», «Crime of the Week», «Mission» и т.п. — чтобы было сразу понятно, что это повторяющаяся структура. Ensemble: всегда `Driver: Theme` (напр. "Jon: Honor", "Cersei: Power"). Остальные: добавляй driver когда один персонаж ведёт несколько линий.
